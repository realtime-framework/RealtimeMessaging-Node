        ...
        if (!isConnected) {
            ...
            if (ortc.sockjs != null) {
                ...
                if (userPerms && userPerms != null && !hashPerm) {
                    ...
                    if(ortc.sockjs && ortc.sockjs.isConnected){
                        ortc.sockjs.send(JSON.stringify('subscribefilter;' + ortc.appKey + ';' + ortc.authToken + ';' + channel + ';' + hashPerm + ';' + filter));
                    }
                }
            }
        }
        ...
        if(!options) {
            ...
            if (ortc.sockjs != null) {
                ...
                if (userPerms && userPerms != null && !hashPerm) {
                    ...
                    if(ortc.sockjs && ortc.sockjs.isConnected){
                        ortc.sockjs.send(JSON.stringify('subscribeoptions;' + ortc.appKey + ';' + ortc.authToken + ';' + channel + ';' + subscriberId + ';;;' + hashPerm + ';' + filter));
                    }
                }
            }
        }
        ...
        */
        ...
        if (!isConnected || ortc.sockjs == null) {
            ...
            if (userPerms && userPerms != null && !hashPerm) {
                ...
                if (messageParts.length == 1) {
                    if(ortc.sockjs && ortc.sockjs.isConnected){
                        ortc.sockjs.send(JSON.stringify('publish;' + ortc.appKey + ';' + ortc.authToken + ';' + channel + ';' + ttl + ';' + hashPerm + ';' + messageId + '_1-1_' + messageParts[0]));
                    }
    ...
    var startHeartBeatInterval = function(self){
      var condition  =!self.heartbeatInterval && heartbeatActive;
        if(!self.heartbeatInterval && heartbeatActive){
            self.sockjs.send(JSON.stringify("b"));
    ...
    var createSocketConnection = function (connectionUrl) {
        if (ortc.sockjs == null) {
            ...
            ortc.sockjs.onopen = function () {
                ...
                if(ortc.sockjs && ortc.sockjs.isConnected){
                    ortc.sockjs.send(validateMessage)
                }
                        ...
                        switch (op) {
                                        ...
                                        if (subscribedChannels[key].subscribeOnReconnected == true && (subscribedChannels[key].isSubscribing || subscribedChannels[key].isSubscribed)) {
                                            ...
                                            if(ortc.sockjs && ortc.sockjs.isConnected){
                                                if(subscribedChannels[key].withOptions) {
                                                    ...
                                                    ortc.sockjs.send(JSON.stringify('subscribeoptions;' + ortc.appKey + ';' + ortc.authToken + ';' + key + ';' + subscriberId + ';;;' + hashPerm + ';' + filter));
                                                } else if(subscribedChannels[key].filter) {
                                                    ortc.sockjs.send(JSON.stringify('subscribefilter;' + ortc.appKey + ';' + ortc.authToken + ';' + key + ';' + hashPerm + ';' + subscribedChannels[key].filter));
                                                } else {
                            ...
                            default:
                                ...
                                if(messageId && seqId) {
                                    ...
                                    if(ortc.sockjs) {
                                        ortc.sockjs.send(JSON.stringify('ack;' + ortc.appKey + ';' + channel + ';' + messageId + ';' + seqId + ';' + haveAllParts));
                                    }   
                                }
                                break;
                        }
